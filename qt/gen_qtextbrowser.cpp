...
public:
	...
	// cgo.Handle value for overwritten implementation
	miqt_callback_handle<miqt_exec_callback_handle_release_QTextBrowser> handle__setSource;

	// Subclass to allow providing a Go implementation
	virtual void setSource(const QUrl& name) override {
		if (!handle__setSource) {
		...
		miqt_exec_callback_QTextBrowser_setSource(this, handle__setSource.value(), sigval1);
...
void* QTextBrowser_connect_highlightedWithQString(QTextBrowser* self, intptr_t slot) {
	auto slot_handle = std::make_shared<miqt_callback_handle<miqt_exec_callback_handle_release_QTextBrowser>>(slot);
	return new QMetaObject::Connection(QTextBrowser::connect(self, static_cast<void (QTextBrowser::*)(const QString&)>(&QTextBrowser::highlighted), self, [slot_handle](const QString& param1) {
		intptr_t slot = slot_handle->value();
...
bool QTextBrowser_override_virtual_setSource(void* self, intptr_t slot) {
	...
	self_cast->handle__setSource = std::move(slot_handle);
	return true;
}
