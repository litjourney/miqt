...
func (this *QGraphicsLayoutItem) OnIsEmpty(slot func(super func() bool) bool) {
	...
	ok := C.QGraphicsLayoutItem_override_virtual_isEmpty(unsafe.Pointer(this.h), slotHandle)
	if !ok {
		panic("miqt: can only override virtual methods for directly constructed types")
	}
}
...
type miqtVirtualCallback_QGraphicsLayoutItem_sizeHint struct {
...
func (this *QGraphicsLayoutItem) OnSizeHint(slot func(which SizeHint, constraint *QSizeF) *QSizeF) {
	var slotHandle C.intptr_t
	if slot != nil {
		slotHandle = C.intptr_t(cgo.NewHandle(miqtVirtualCallback_QGraphicsLayoutItem_sizeHint{callback: slot}))
	}
...
func (this *QGraphicsLayoutItem) OnSizeHintOwned(slot func(which SizeHint, constraint *QSizeF) *QSizeF) {
	var slotHandle C.intptr_t
	if slot != nil {
		slotHandle = C.intptr_t(cgo.NewHandle(miqtVirtualCallback_QGraphicsLayoutItem_sizeHint{callback: slot, ownsReturn: true}))
	}
	ok := C.QGraphicsLayoutItem_override_virtual_owned_sizeHint(unsafe.Pointer(this.h), slotHandle)
	if !ok {
		panic("miqt: can only override virtual methods for directly constructed types")
	}
}
...
func miqt_exec_callback_QGraphicsLayoutItem_sizeHint(self *C.QGraphicsLayoutItem, cb C.intptr_t, which C.int, constraint *C.QSizeF) *C.QSizeF {
	callbackData, ok := cgo.Handle(cb).Value().(miqtVirtualCallback_QGraphicsLayoutItem_sizeHint)
	if !ok {
		panic("miqt: callback of non-callback type (heap corruption?)")
	}
