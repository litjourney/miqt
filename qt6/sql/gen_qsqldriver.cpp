...
void miqt_exec_callback_QSqlDriver_notification(intptr_t, struct miqt_string, int, QVariant*);
...
public:
	...
	// cgo.Handle value for overwritten implementation
	miqt_callback_handle<miqt_exec_callback_handle_release_QSqlDriver> handle__maximumIdentifierLength;

	// Subclass to allow providing a Go implementation
	virtual int maximumIdentifierLength(QSqlDriver::IdentifierType type) const override {
		if (!handle__maximumIdentifierLength) {
			return QSqlDriver::maximumIdentifierLength(type);
		}

		QSqlDriver::IdentifierType type_ret = type;
		int sigval1 = static_cast<int>(type_ret);
		int callback_return_value = miqt_exec_callback_QSqlDriver_maximumIdentifierLength(this, handle__maximumIdentifierLength.value(), sigval1);
...
bool QSqlDriver_override_virtual_maximumIdentifierLength(void* self, intptr_t slot) {
	...
	self_cast->handle__maximumIdentifierLength = std::move(slot_handle);
	return true;
}
