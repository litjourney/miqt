...
type miqtVirtualCallback_QwtScaleDraw_label struct {
...
func (this *QwtScaleDraw) OnLabel(slot func(super func(param1 float64) *QwtText, param1 float64) *QwtText) {
	var slotHandle C.intptr_t
	if slot != nil {
		slotHandle = C.intptr_t(cgo.NewHandle(miqtVirtualCallback_QwtScaleDraw_label{callback: slot}))
	}
...
func (this *QwtScaleDraw) OnLabelOwned(slot func(super func(param1 float64) *QwtText, param1 float64) *QwtText) {
	var slotHandle C.intptr_t
	if slot != nil {
		slotHandle = C.intptr_t(cgo.NewHandle(miqtVirtualCallback_QwtScaleDraw_label{callback: slot, ownsReturn: true}))
	}
	ok := C.QwtScaleDraw_override_virtual_owned_label(unsafe.Pointer(this.h), slotHandle)
	if !ok {
		panic("miqt: can only override virtual methods for directly constructed types")
	}
}
...
func miqt_exec_callback_QwtScaleDraw_label(self *C.QwtScaleDraw, cb C.intptr_t, param1 C.double) *C.QwtText {
	callbackData, ok := cgo.Handle(cb).Value().(miqtVirtualCallback_QwtScaleDraw_label)
	if !ok {
		panic("miqt: callback of non-callback type (heap corruption?)")
	}
