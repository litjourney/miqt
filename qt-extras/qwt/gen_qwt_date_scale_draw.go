...
type miqtVirtualCallback_QwtDateScaleDraw_label struct {
...
func (this *QwtDateScaleDraw) OnLabel(slot func(super func(param1 float64) *QwtText, param1 float64) *QwtText) {
	...
	if slot != nil {
		slotHandle = C.intptr_t(cgo.NewHandle(miqtVirtualCallback_QwtDateScaleDraw_label{callback: slot}))
	}
...
func (this *QwtDateScaleDraw) OnLabelOwned(slot func(super func(param1 float64) *QwtText, param1 float64) *QwtText) {
	var slotHandle C.intptr_t
	if slot != nil {
		slotHandle = C.intptr_t(cgo.NewHandle(miqtVirtualCallback_QwtDateScaleDraw_label{callback: slot, ownsReturn: true}))
	}
	ok := C.QwtDateScaleDraw_override_virtual_owned_label(unsafe.Pointer(this.h), slotHandle)
	if !ok {
		panic("miqt: can only override virtual methods for directly constructed types")
	}
}
...
func miqt_exec_callback_QwtDateScaleDraw_label(self *C.QwtDateScaleDraw, cb C.intptr_t, param1 C.double) *C.QwtText {
	callbackData, ok := cgo.Handle(cb).Value().(miqtVirtualCallback_QwtDateScaleDraw_label)
	if !ok {
